    ...
    return (
      <div className="min-h-screen bg-[#08080c] flex items-center justify-center px-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass rounded-2xl p-8 max-w-md w-full text-center border border-white/5">
          ...
          <p className="text-sm text-slate-400 mb-6">We've sent a confirmation link to <span className="text-white font-medium">{email}</span>. Click it to activate your account.</p>
          <Link to="/login" className="glow-btn rounded-xl px-6 py-2.5 text-sm font-semibold text-white inline-flex items-center gap-2">
    ...
    <div className="min-h-screen bg-[#08080c] flex items-center justify-center px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8 border border-white/5">
          ...
          <p className="text-sm text-slate-400 mb-6">Start analyzing data in minutes — free</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <span className="animate-pulse">Creating account...</span> : <>Create Account <ArrowRight size={14} /></>}
            </button>
          </form>
          ...
          <button onClick={async () => { try { await signInWithGoogle() } catch (err) { setError(err.message || 'Google signup failed.') } }}
